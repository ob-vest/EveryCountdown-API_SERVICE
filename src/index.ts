...
app.get("/search", async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT'movie' AS type,headline,subheadline, id FROM movie WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'tv' AS type,headline,subheadline, id FROM tv WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'anime' AS type,headline,subheadline, id FROM anime WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'tech' AS type,headline,subheadline, id FROM tech WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'game' AS type,headline,subheadline, id FROM game WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'politics' AS type,headline,subheadline, id FROM politics WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'sport' AS type,headline,subheadline, id FROM sport WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)UNION SELECT'holiday' AS type,headline,subheadline, id FROM holiday WHERE to_tsvector('english',headline||' '||subheadline)@@ to_tsquery('english',$1)`,
      [search]
    );
    res.send(rows);
  } catch (error) {
